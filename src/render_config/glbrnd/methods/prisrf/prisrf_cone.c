...
/*
  ======================================================================
  Functions:
	DtFlag drr_glbrnd_prisrf_rayint_cone (prisrf,ray,intersect)
	int drr_glbrnd_prisrf_world_boundary_cone (lcstowcsmat,xmin,ymin,zmin,xmax,ymax,zmax)
	DtFlag drr_glbrnd_prisrf_overlap_spacebox_cone (lcstowcsmat,boxminmax)
    ...
    struct intersect *intersect)
{
    ...
    b = 2. * (rayx0*rayxdir + rayy0*rayydir - rayz0*rayzdir + rayzdir);
    c = rayx0*rayx0 + rayy0*rayy0 - rayz0*rayz0 + 2*rayz0 - 1.;
    ...
    if (intersectflag) {
   	...
   	intersect->lcsnrm[1] = ynorm;
   	intersect->lcsnrm[2] = znorm;
   	dor_math_renormalize_fast(intersect->lcsnrm);
    }
...
DtFlag drr_glbrnd_prisrf_overlap_spacebox_cone (
    DtMatrix4x4 lcstowcsmat,
    DtReal boxminmax[6])
{
    return drr_glbrnd_prisrf_overlap_lcsbox_spacebox
	       (-1.,-1.,0., 1.,1.,1., lcstowcsmat, boxminmax);
}
