...
/*
  ======================================================================
  Functions:
	int drr_glbrnd_vartristrip_initialize_objects (object)
...
void drr_glbrnd_vartristrip_initialize_objects (dot_object *object)
{
   ...
   normal = (vartristrip->vertex_normal != NULL);
   ...
   for (basevtxno=0; basevtxno<vartristrip->vertex_count-2; basevtxno++) {
	...
	vertex3 = basevtxno+2;
  	...
  	if (!dor_check_degen_triangle_vtx_values(
            vartristrip->vertex_location[vertex1],
            vartristrip->vertex_location[vertex2],
            vartristrip->vertex_location[vertex3]))
            continue;

	dor_math_pnttrns_fast(vartristrip->vertex_location[vertex1],
			lcstowcsmat, wcsloc[0]);

	dor_math_pnttrns_fast(vartristrip->vertex_location[vertex2],
			lcstowcsmat, wcsloc[1]);

	dor_math_pnttrns_fast(vartristrip->vertex_location[vertex3],
			lcstowcsmat, wcsloc[2]);

   	if (normal) {
	    dor_math_vectrns_fast(vartristrip->vertex_normal[vertex1],
			lcstowcsmat, wcsnrm[0]);

	    dor_math_renormalize_fast (wcsnrm[0]);

	    dor_math_vectrns_fast(vartristrip->vertex_normal[vertex2],
			lcstowcsmat, wcsnrm[1]);

	    dor_math_renormalize_fast (wcsnrm[1]);

	    dor_math_vectrns_fast(vartristrip->vertex_normal[vertex3],
