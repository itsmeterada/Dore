...
/*
  ...
  Functions:
	...
	dor_global_spcclr_set_value 
	dor_global_spcclr_push_value 
	dor_global_spcclr_pop_value 
	dor_global_spcclr_pop_group 
	dor_global_spcclr_get_value 
	dor_global_spcclr_get_data_pointer 
	dor_global_spcclr_update_execute 
	dor_global_spcclr_destroy_element 

  ======================================================================
 */
#include "global.h"

dot_object *doe_global_spcclr_object;
DtInt doe_global_spcclr_class_id;
/*
 ======================================================================
 */
...
static DtMethodEntry methods[] = {
    { DcMethodInqGlbAttVal, (DtMethodPtr)dor_global_spcclr_get_value }
};
...
void dor_global_spcclr_initialize (void)
{
    ...
    doe_global_spcclr_class_id = dor_class_install (DcTypeAttSpecularColor, 
		"AttSpecularColor", 1, methods,DcNullPtr);

    gas = dor_gas_create(dor_global_spcclr_destroy_element);

    doe_global_spcclr_object = dor_object_create(doe_global_spcclr_class_id,gas);

    initialcolor[0] = 1.;
    initialcolor[1] = 1.;
    initialcolor[2] = 1.;

    dor_global_spcclr_push_value(initialcolor);
}
/*
 ======================================================================
 */

void dor_global_spcclr_set_value (
    DtColorRGB color)
{
    DtColorRGB *dataptr;
	...
	first on the current group stack), then a new specular colour 
	is created and pushed on the stack.  Otherwise, the current 
	specular colour is loaded with the user specified colour.  */

    if (dor_gas_test_set_value(doe_global_spcclr_object->data)){
	dor_global_spcclr_push_value(color);
    }
    else{
	dataptr = dor_global_spcclr_get_data_pointer();
...
void dor_global_spcclr_push_value (
    DtColorRGB color)
{
    DtColorRGB *newcolor;
    ...
    if (dor_gas_push_value(doe_global_spcclr_object->data,newcolor)){
	currentgroup = dor_group_get_current_group();

	dor_group_record_attribute(currentgroup,dor_global_spcclr_pop_group);
    }
...
DtFlag dor_global_spcclr_pop_value (void)
{
    ...
    popflag = dor_gas_pop_value (doe_global_spcclr_object->data,
				 (DtPtr)(&color));

    dor_global_spcclr_destroy_element(color);
...
{
    dor_gas_pop_group(doe_global_spcclr_object->data);
...
DtFlag dor_global_spcclr_get_value (
    ...
    DtReal color[3])
{
    ...
    getflag = dor_gas_get_value (doe_global_spcclr_object->data,
				 (DtPtr)(&currentcolor));

    if (!getflag){
	DDerror (ERR_NO_CURRENT_VALUE,
		    "dor_global_spcclr_get_value", DcNullPtr);
...
DtColorRGB *dor_global_spcclr_get_data_pointer (void)
{
    ...
    getflag = dor_gas_get_value(doe_global_spcclr_object->data,&dataptr);

    if (!getflag){
	DDerror (ERR_NO_CURRENT_VALUE,
		    "dor_global_spcclr_get_data_pointer", DcNullPtr);
...
void dor_global_spcclr_update_execute (void)
{
    ...
    executemethod = dor_object_get_current_method(doe_global_spcclr_object);
...
void dor_global_spcclr_destroy_element (
    DtColorRGB *color)
{
