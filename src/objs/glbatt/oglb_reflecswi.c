...
/*
  ...
  Functions:
	...
	dor_global_reflecswi_set_value 
	dor_global_reflecswi_push_value 
	dor_global_reflecswi_pop_value 
	dor_global_reflecswi_pop_group 
	dor_global_reflecswi_get_value 
	dor_global_reflecswi_get_data_pointer 
	dor_global_reflecswi_update_execute 
	dor_global_reflecswi_destroy_element 

  ======================================================================
 */
#include "global.h"

dot_object *doe_global_reflecswi_object;
DtInt doe_global_reflecswi_class_id;
/*
 ======================================================================
 */
...
static DtMethodEntry methods[] = {
    { DcMethodInqGlbAttVal, (DtMethodPtr)dor_global_reflecswi_get_value }
};
...
void dor_global_reflecswi_initialize (void)
{
    ...
    doe_global_reflecswi_class_id =
	dor_class_install (DcTypeAttReflectionSwitch, "AttReflectionSwitch",
			   1, methods, DcNullPtr);

    gas = dor_gas_create(dor_global_reflecswi_destroy_element);

    doe_global_reflecswi_object = dor_object_create(doe_global_reflecswi_class_id,
						    gas);

    dor_global_reflecswi_push_value(DcOff);
}
/*
 ======================================================================
 */

void dor_global_reflecswi_set_value (DtSwitch switchvalue)
{
    DtSwitch *dataptr;
    ...
    if (dor_gas_test_set_value(doe_global_reflecswi_object->data)){
	dor_global_reflecswi_push_value(switchvalue);
    }
    else{
	dataptr = dor_global_reflecswi_get_data_pointer();
...
void dor_global_reflecswi_push_value (DtSwitch switchvalue)
{
    ...
    *newswitchvalue = switchvalue;
    ...
    if (dor_gas_push_value(doe_global_reflecswi_object->data,newswitchvalue)){
	currentgroup = dor_group_get_current_group();

	dor_group_record_attribute(currentgroup,dor_global_reflecswi_pop_group);
    }
...
DtFlag dor_global_reflecswi_pop_value (void)
{
    ...
    popflag = dor_gas_pop_value(doe_global_reflecswi_object->data,&switchvalue);

    dor_global_reflecswi_destroy_element(switchvalue);
...
{
    dor_gas_pop_group(doe_global_reflecswi_object->data);
...
DtFlag dor_global_reflecswi_get_value (DtSwitch *switchvalue)
{
    DtFlag getflag;
    DtPtr currentswitchvalue;

    getflag = dor_gas_get_value(doe_global_reflecswi_object->data,
				&currentswitchvalue);

    if (!getflag){
	DDerror (ERR_NO_CURRENT_VALUE,
		    "dor_global_reflecswi_get_value", DcNullPtr);
...
DtSwitch *dor_global_reflecswi_get_data_pointer (void)
{
    ...
    getflag = dor_gas_get_value(doe_global_reflecswi_object->data,&dataptr);

    if (!getflag){
	DDerror (ERR_NO_CURRENT_VALUE,
		    "dor_global_reflecswi_get_data_pointer", DcNullPtr);
...
void dor_global_reflecswi_update_execute (void)
{
    ...
    executemethod = dor_object_get_current_method(doe_global_reflecswi_object);
...
void dor_global_reflecswi_destroy_element (DtSwitch *element)
{
