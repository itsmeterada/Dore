...
/*
 ...
 Functions:
   ...
   void dpr_radiosity_vartristrip_show_render(object)

 ======================================================================
 */

void dpr_radiosity_vartristrip_show_render (
    dot_object *object)
{
    ...
    renderable = DDcondex_QueryRender(DsInqClassId("DoVarTriangleStrip"));
    ...
    if (radswi == DcOff) {
		...
		for (i=0; i<vartristrip->vertex_count; i++) {

            /* if vertex colors are present, blend with radiosities */
			if (vartristrip->vertex_color != DcNullPtr) {
                add_data->vertexRad[i][0] *= vartristrip->vertex_color[i][0];
                add_data->vertexRad[i][1] *= vartristrip->vertex_color[i][1];
                add_data->vertexRad[i][2] *= vartristrip->vertex_color[i][2];
            }
        ...
        current_vertex_color = vartristrip->vertex_color;

        /* swap for the radiosity results */
        vartristrip->vertex_color = add_data->vertexRad;
        ...
        vartristrip->vertex_color = current_vertex_color;
