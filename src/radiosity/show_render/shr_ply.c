...
/*
 ...
 Functions:
   ...
   void dpr_radiosity_ply_show_render(object)

 ======================================================================
 */

void dpr_radiosity_ply_show_render (
    dot_object *object)
{
    ...
    renderable = DDcondex_QueryRender(DsInqClassId("DoPolygon"));
    ...
    if (radswi == DcOff) {
        ...
        if ((reptype == DcOutlines) ||
            ((reptype != DcOutlines) && (ply->alt_object_tris == DcNullPtr))) {
           ...
           if (!renderable)
            ...
            exec_rtn = DDobject_InqMethod(ply->alt_object_tris,
                           dpe_rad_show_data.show_render_method_id);
           (*exec_rtn)(ply->alt_object_tris);
       }
