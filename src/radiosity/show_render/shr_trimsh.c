...
/*
 ...
 Functions:
   ...
   void dpr_radiosity_trimsh_show_render(object)

 ======================================================================
 */

void dpr_radiosity_trimsh_show_render (
    dot_object *object)
{
    ...
    renderable = DDcondex_QueryRender(DsInqClassId("DoTriangleMesh"));
    ...
    if (radswi == DcOff) {
        ...
        current_vertextype = trimsh->vertextype;

        for (vtx=0; vtx<trimsh->vertex_count; vtx++) {
           sumrad[0] = sumrad[1] = sumrad[2] = 0.0;
           numtri = 0;
           for (j=0; j<trimsh->triangle_count; j++) {
               for (k=0; k<3; k++) {
                   if (trimsh->triangle_vertexlist[j][k] == vtx) {
            ...
            if (numtri != 0) {
               ...
               if (current_vertextype == DcLocClr || 
                   ...
                   add_data->vertexRad[vtx][0] *= trimsh->vertex_color[vtx][0];
                   add_data->vertexRad[vtx][1] *= trimsh->vertex_color[vtx][1];
                   add_data->vertexRad[vtx][2] *= trimsh->vertex_color[vtx][2];
               }
        ...
        current_vertex_color = trimsh->vertex_color;

        /* swap for the radiosity results */
        trimsh->vertex_color = add_data->vertexRad;

        /* change vertex type to indicate presence of vertex colors */
        trimsh->vertextype = trimsh->vertextype | DcLocClr;
        ...
        /* set inetrp type to vertex shade to interpolate v radiosities*/
        ...
        trimsh->vertex_color = current_vertex_color;

        trimsh->vertextype = current_vertextype;
   }
