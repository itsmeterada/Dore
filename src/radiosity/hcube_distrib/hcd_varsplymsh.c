...
/*
 ...
 Functions:
    ...
    void dpr_radiosity_varsimplymsh_hcube_distrib(object)

 ======================================================================
 */

void dpr_radiosity_varsimplymsh_hcube_distrib (
    dot_object *object)
{
    ...
    for (i=0; i<varsimplymsh->polygon_count; i++) {
        ...
        /* ignore if this is the shooting patch or if ffactor is zero */
        if ((int_elem_color != dpe_rad_brightest_patch.intElemColor) &&
            ...
            if (add_data->patchArea[i] != 0.)
                ...
                DDerror(ERR_RAD_ZERO_AREA,
                  "dpr_radiosity_varsimplymsh_hcube_distrib", DcNullPtr);
