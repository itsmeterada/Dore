...
/*
  ======================================================================
  Functions:
	ddt_g4vll_drom_trilst ddr_g4vll_drom_trilst_render (object)
        void ddr_g4vll_drom_trilst_print (object, data)
        void ddr_g4vll_drom_trilst_delete (object, data)
        void ddr_g4vll_drom_trilst_render (object)
...
/*
  ======================================================================
 */
ddt_g4vll_drom_trilst *ddr_g4vll_drom_trilst_create (
    DtObjectStructure *object)
{
    struct trilst *trilst;
    ddt_g4vll_drom_trilst *drom_trilst;
    int i;

    trilst = (struct trilst *)(object->data);

    drom_trilst =  (ddt_g4vll_drom_trilst *)
            DDspace_Allocate(sizeof(ddt_g4vll_drom_trilst));

    if (trilst->uv_count > 0) {
        drom_trilst->bboxuv = (ddt_g4vll_drom_bboxuv *)DDspace_Allocate
                (trilst->uv_count *sizeof(ddt_g4vll_drom_bboxuv));

        for (i=0; i< trilst->uv_count; i++) {
            drom_trilst->bboxuv[i].initialized = DcFalse;
        }
    } else {
        drom_trilst->bboxuv = (ddt_g4vll_drom_bboxuv *)0;
    }

    if (trilst->uvw_count > 0) {
        drom_trilst->bboxuvw = (ddt_g4vll_drom_bboxuvw *)DDspace_Allocate
                (trilst->uvw_count *sizeof(ddt_g4vll_drom_bboxuvw));

        for (i=0; i< trilst->uvw_count; i++) {
            drom_trilst->bboxuvw[i].initialized = DcFalse;
        }
    } else {
        drom_trilst->bboxuvw = (ddt_g4vll_drom_bboxuvw *)0;
    }
...
/*
  ======================================================================
 */
void ddr_g4vll_drom_trilst_print (
    DtObject object,
    DtInt *data)
{
}
/*
  ======================================================================
 */
void ddr_g4vll_drom_trilst_delete (
    DtObject object,
    ddt_g4vll_drom_trilst *data)
{
    if (data != (ddt_g4vll_drom_trilst *)0) {
...
/*
 ======================================================================
 */
void ddr_g4vll_drom_trilst_render (
    dot_object *object)
{
    extern DtInt *dde_g4vll_triangle_list_edge_enable;
    extern DtRealCouple *dde_g4vll_triangle_list_bboxuv;
    extern DtRealTriple *dde_g4vll_triangle_list_bboxuvw;
    struct trilst *trilst;
    ddt_g4vll_drom_trilst *drom_trilst;
    ...
    drom_trilst = (ddt_g4vll_drom_trilst *)object->additional_data[dataindex];

    /*
     * Store the object's additional data globally for its output routine.
     */
    dde_g4vll_drom_trilst = drom_trilst;

    if (trilst->uv_count > 0 && dde_g4vll_drom_data.texture_2D_valid) {
        idx = trilst->uv_count > dde_g4vll_drom_data.map_diffuse_color_uv_index ?
        ...
        dde_g4vll_triangle_list_bboxuv = bboxuv->bbox;
    }

    if (trilst->uvw_count > 0 && dde_g4vll_drom_data.texture_3D_valid) {
        idx = trilst->uvw_count > dde_g4vll_drom_data.map_diffuse_color_uvw_index ?
        ...
        dde_g4vll_triangle_list_bboxuvw = bboxuvw->bbox;
    }
	...
	dde_g4vll_triangle_list_edge_enable = trilst->edge_enable;
    }
    ...
    dde_g4vll_triangle_list_edge_enable = (DtInt *)0;
    dde_g4vll_triangle_list_bboxuv = (DtRealCouple *)0;
    dde_g4vll_triangle_list_bboxuvw = (DtRealTriple *)0;
    dde_g4vll_drom_trilst = (ddt_g4vll_drom_trilst *)0;
}
/*
 ======================================================================
 */
